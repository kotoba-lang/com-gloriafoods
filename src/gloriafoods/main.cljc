(ns gloriafoods.main
  "Kotodama WASM entrypoint for the Gloriafoods clean-room actor (L4) — Clojure port.
  ...
  facts (`gloriafoods.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gloriafoods")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"      :plural "orders"      :id-prefix "gloriafo_ord"
    :fields [:customerId :merchantId :total :status]    :required [:total :status]
    :coerce {:total :float}               :refs {:merchantId "Merchant"}}
   {:entity "Courier"    :plural "couriers"    :id-prefix "gloriafo_cou"
    :fields [:name :vehicle :lat :lon :online]          :required [:name :vehicle]
    :coerce {:lat :float :lon :float :online :bool}     :refs {}}
   {:entity "Delivery"   :plural "deliveries"  :id-prefix "gloriafo_del"
    :fields [:orderId :courierId :status :etaMin]       :required [:status :etaMin]
    :coerce {:etaMin :int}                :refs {:orderId "Order" :courierId "Courier"}}
   {:entity "Merchant"   :plural "merchants"   :id-prefix "gloriafo_mer"
    :fields [:name :cuisine :lat :lon]                  :required [:name :cuisine]
    :coerce {:lat :float :lon :float}     :refs {}}
   {:entity "Route"      :plural "routes"      :id-prefix "gloriafo_rou"
    :fields [:deliveryId :distanceKm :durationMin]      :required [:distanceKm :durationMin]
    :coerce {:distanceKm :float :durationMin :int}      :refs {:deliveryId "Delivery"}}
   {:entity "Tracking"   :plural "trackings"   :id-prefix "gloriafo_tra"
    :fields [:deliveryId :lat :lon :recordedAt]         :required [:lat :lon]
    :coerce {:lat :float :lon :float}     :refs {:deliveryId "Delivery"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"gloriafoods.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gloriafoods-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gloriafoods-compat/ready)
